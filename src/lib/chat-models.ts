...
//
// `deepseek-flash` (DeepSeek V4 Flash) and `deepseek-pro` (DeepSeek V4 Pro) reach
// chat-service's third provider path: the Vercel AI Gateway, whose provider slug is
// `vercel` (chat-service #376). Both aliases and the slug are chat-service's spelling,
// read off its own schema — never invented here. That path is TEXT-IN / TEXT-OUT only
// — chat-service 400s on `webSearch` / `imageUrl`. Neither of this service's two
// generation paths sends either field, so nothing here depends on a capability the
// models lack. The one caveat is JSON mode: `/generate` forwards a `responseSchema`,
// and neither DeepSeek V4 model advertises `response_format` support, so the gateway
// may pass the schema to a provider that ignores it. That is best-effort but never
// silent — chat-service's JSON parse fails loud (502) on output it cannot read, so a
// malformed sequence is an error, not a half-filled email. The free-text pitch route
// sends no schema and is unaffected.
