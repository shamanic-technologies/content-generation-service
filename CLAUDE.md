...
`POST /generate` and `POST /generate-expert-quote-pitch` accept an optional `model` field — a version-free alias from `CHAT_MODELS` (`haiku`, `sonnet`, `opus`, `flash-lite`, `flash`, `flash-pro`, `pro`, `deepseek-flash`). Caller picks the LLM per request; omitted → `DEFAULT_MODEL` (`pro`/google), byte-identical to before the field existed.

- **Provider is derived, never sent by the caller.** The 8 aliases are unique across providers, so `MODEL_TO_PROVIDER` maps alias→provider (`chat-models.ts`). No `provider` field on the wire → the provider/model-mismatch error class can't happen.
- **`deepseek-flash` → provider `vercel`.** DeepSeek V4 Flash reaches chat-service's third provider path, the Vercel AI Gateway (chat-service #376). The alias spelling and the provider slug are chat-service's, not ours — do NOT invent either. That path is **text-in / text-out only**: chat-service 400s on `webSearch` / `imageUrl`, and neither of this service's two generation paths sends either field, so nothing here needs a capability the model lacks. The one caveat is JSON mode — `/generate` sends a `responseSchema` and DeepSeek V4 Flash does not advertise `response_format` support, so the gateway may hand the schema to a provider that ignores it. Best-effort but never silent: chat-service's JSON parse fails loud (502) on unreadable output, so a bad sequence is an error, not a half-filled email. The pitch route sends no schema and is unaffected.
- **`/generate` response schema is provider-conditional.** `GENERATE_RESPONSE_SCHEMA` (permissive) is sent for google AND vercel (the gateway forwards the schema verbatim into an OpenAI-compatible `response_format` and imposes no `additionalProperties` requirement); `GENERATE_RESPONSE_SCHEMA_STRICT` (`additionalProperties:false` on the object AND `emails.items`) only for anthropic — anthropic's structured-output API 400s on permissive schemas, google ignores the keyword. The google path is unchanged. The pitch route is free-text (no responseSchema) → all 8 work as-is.
