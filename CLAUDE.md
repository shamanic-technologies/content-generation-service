...
- **Vendor ACCOUNT state is not a routing concern, and there is NO fallback.** As of 2026-08-16 DeepSeek is funded and answering; Z.ai and Moonshot authenticate but carry no balance, so they answer **429 "please recharge"**, and Moonshot's two models have no costs-service row yet (their vendor prices are not published), so a Kimi call fails at cost declaration even once funded. Both are account/catalog states owned elsewhere (billing / costs-service) — do NOT "fix" a 429 by rerouting to a funded vendor or by re-mapping the alias. An unfunded vendor fails loud, and the thrown error names the provider + model (`chat-service /complete failed for provider 'zai' model 'glm-pro': 429 - …`) so the failure reads as an account problem, not an anonymous chat-service outage.
