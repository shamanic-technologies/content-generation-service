...
- **`deepseek-flash` + `deepseek-pro` → provider `vercel`.** DeepSeek V4 Flash and V4 Pro reach chat-service's third provider path, the Vercel AI Gateway (chat-service #376). Both alias spellings and the provider slug are chat-service's, not ours — read them off its `/complete` schema, do NOT invent either. That path is **text-in / text-out only**: chat-service 400s on `webSearch` / `imageUrl`, and neither of this service's two generation paths sends either field, so nothing here needs a capability the models lack. The one caveat is JSON mode — `/generate` sends a `responseSchema` and neither DeepSeek V4 model advertises `response_format` support, so the gateway may hand the schema to a provider that ignores it. Best-effort but never silent: chat-service's JSON parse fails loud (502) on unreadable output, so a bad sequence is an error, not a half-filled email. The pitch route sends no schema and is unaffected. Separately, the Vercel account's free gateway tier serves only a subset of the catalog and 403s on both DeepSeek models until credits are purchased — expected, handled outside this service, not a reason to add a fallback.
