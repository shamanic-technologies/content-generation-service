...
describe("traceEvent", () => {
  ...
  it("should POST event to runs-service", async () => {
    ...
    await traceEvent(
      "run-123",
      { service: "content-generation-service", event: "test-event", detail: "details here" },
    ...
    expect(body.service).toBe("content-generation-service");
  ...
  it("should skip when RUNS_SERVICE_URL is not set", async () => {
    ...
    await traceEvent(
      "run-123",
      { service: "content-generation-service", event: "test" },
      {}
    );
