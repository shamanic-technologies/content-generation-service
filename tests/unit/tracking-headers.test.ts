...
describe("tracking headers (x-campaign-id, x-brand-id, x-workflow-slug, x-feature-slug, x-audience-id)", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  describe("POST /generate", () => {
    ...
    it("uses header values when body fields are absent", async () => {
      await request(app)
        ...
        .set("X-Audience-Id", "audience-from-header")
        .send({
          type: "email",
      ...
      expect(mockInsertValues[0]).toEqual(
        expect.objectContaining({
          ...
          audienceId: "audience-from-header",
        })
      );
    ...
    it("threads body.offerId into the identity — workflow-service sends it in the body", async () => {
      await request(app)
        .post("/generate")
        .set("X-Org-Id", "org-123")
        .set("X-User-Id", "user-456")
    ...
    it("ignores an x-offer-id header — there is no header fallback for the offer", async () => {
      await request(app)
        .post("/generate")
        .set("X-Org-Id", "org-123")
        .set("X-User-Id", "user-456")
      ...
      expect(mockCreateRun).toHaveBeenLastCalledWith(
        expect.anything(),
        expect.objectContaining({ offerId: undefined })
      );
    });
  });
});
